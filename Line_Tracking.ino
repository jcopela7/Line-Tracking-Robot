    ...
    case 1:    //Robot Run after 3 seconds
      {
        if (bt_3_S_Time_Up)
        {
          ...
          else if (operatingMode == 's'){scanForLight();fuckedYourBitch = true;}
          else if (operatingMode == 'f'){finishMeOff();}
          else if (operatingMode == 'v'){victoryDanceOfChampions();}
...
void checkForOpMode() //maybe add a parameter to indicate which way it is turning first, left or right
{
  ...
  {
    if (!fuckedYourBitch){operatingMode = 's';}
...
void scanForLight()////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////scanning
{
    ...
    else if (leftSenseOn && middleSenseOn && rightSenseOn)
    {
      ...
      {ui_Right_Motor_Speed = 1600;ui_Left_Motor_Speed = 1600;writeToMotors();}
...
void disableArmMotors()
{
  servo_GripMotor.detach();
  servo_ArmMotor.detach();
}

void victoryDanceOfChampions()
{
  ui_Right_Motor_Speed = 1500;
  ui_Left_Motor_Speed = 1500;
  encoder_RightMotor.zero();
 ...
 while (encoder_RightMotor.getRawPosition() > -3000) //rotate right until it reaches a certain point
  {
    rotateRight(2100);
    writeToMotors();
    //servo_GripMotor.write(random(ci_Grip_Motor_Closed,ci_Grip_Motor_Open));
  }
  ...
  while (encoder_LeftMotor.getRawPosition() > -3000) //rotate left
  {
    rotateLeft(2100);
    writeToMotors();
    servo_GripMotor.write(random(ci_Grip_Motor_Closed,ci_Grip_Motor_Open));
  ...
  servo_ArmMotor.write(random(ci_Arm_Servo_Retracted,ci_Arm_Servo_Extended));
  servo_GripMotor.write(random(ci_Grip_Motor_Closed,ci_Grip_Motor_Open));
  delay(1000);
  ...
  else if (leftSenseOn && middleSenseOn && rightSenseOn)
  {
    ...
    servo_ArmMotor.attach(ci_Arm_Motor);
      ...
      encoder_LeftMotor.zero();
       ...
       if (encoder_LeftMotor.getRawPosition() >1550)
